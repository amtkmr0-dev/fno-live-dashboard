...
async def _serve_static_page(filename: str):
    """Serve a static HTML page by filename (no-cache to ensure fresh content)."""
...
async def _serve_auth_page(request, filename: str):
    """Serve an HTML page that requires authentication."""
    token = get_session_from_request(request)
    session = validate_session(token)
    if not session:
        raise web.HTTPFound("/login")
    return await _serve_static_page(filename)
...
# --- Auth-required page handlers (explicit routes beat catch-all proxy) ---
async def handle_dashboard_page(request):
    """Serve dashboard_live.html (auth required)."""
    return await _serve_auth_page(request, "dashboard_live.html")


async def handle_sectors_page(request):
    """Serve sectors.html (auth required)."""
    return await _serve_auth_page(request, "sectors.html")


async def handle_admin_page(request):
    """Serve admin.html (auth + admin role required)."""
    token = get_session_from_request(request)
    session = validate_session(token)
    if not session:
        raise web.HTTPFound("/login")
    ...
    return await _serve_static_page("admin.html")


async def handle_paper_trades_page(request):
    """Serve paper_trades.html (auth required)."""
    return await _serve_auth_page(request, "paper_trades.html")


async def handle_paper_page(request):
    """Serve paper.html (auth required)."""
    return await _serve_auth_page(request, "paper.html")
...
def create_app():
    ...
    # App pages — explicit routes (bypass catch-all proxy entirely)
    app.router.add_get("/", handle_dashboard_page)
    app.router.add_get("/sectors", handle_sectors_page)
